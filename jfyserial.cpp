...
Serial::Serial( const string&  hostIp, int hostPort)
:	_hostIp(hostIp),
	_hostPort(hostPort),
	_handle( -1 ),
	_readTimeout( 1 )
...
bool Serial::open()
{
	...
	if ((_handle = socket(AF_INET, SOCK_STREAM, 0)) < 0)
	    {
	    ...
	    if(inet_pton(AF_INET, _hostIp.c_str(), &serv_addr.sin_addr)<=0)
	    {
